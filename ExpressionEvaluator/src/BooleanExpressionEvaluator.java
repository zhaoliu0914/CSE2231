...
public final class BooleanExpressionEvaluator {
    ...
    public static boolean valueOfBoolExpr(Queue<String> tokens) {
        ...
        if (tokens.length() > 0) {
            ...
            if (front.equals("T")) {
                ...
                boolean firstExpr = valueOfBoolExpr(tokens);

                String binaryOp = tokens.dequeue();

                boolean secondExpr = valueOfBoolExpr(tokens);
    ...
    public static void main(String[] args) {
        ...
        while (source.length() > 0) {
            Queue<String> tokenQueue = tokens(source);

            System.out.println("tokenQueue = " + tokenQueue);

            boolean value = valueOfBoolExpr(tokenQueue);
