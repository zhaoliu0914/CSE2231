...
public final class BugsWorldVMInterpreter {
    ...
    public static int nextPrimitiveInstructionAddress(int[] cp, CellState wbs,
        ...
        // 0  JUMP_IF_NOT_TRUE
        // 1  25
        // 2  JUMP_IF_NOT_NEXT_IS_EMPTY
        // 3  7
        // 4  MOVE
        // 5  JUMP
        // 6  2
        // 7  JUMP_IF_NOT_NEXT_IS_ENEMY
        // 8  12
        // 9  INFECT
        // 10 JUMP
        // 11 23
        // 12 JUMP_IF_NOT_NEXT_IS_WALL
        // 13 22
        // 14 JUMP_IF_NOT_RANDOM
        // 15 19
        // 16 TURNLEFT
        // 17 JUMP
        // 18 20
        ...
        // 25 HALT
        //
        // cp = [16, 25, 7, 7, 0, 6, 2, 13, 12, 3, 6, 23, 9, 22, 15, 19, 1, 6, 20, 2, 6, 23, 4, 6, 0, 5]

        int nextPrimInsAddr;

        int instruction = cp[pc];

        boolean isPrimIns = isPrimitiveInstructionByteCode(instruction);

        if (isPrimIns) {
            nextPrimInsAddr = pc;
        } else {
            if (Instruction.JUMP.byteCode() == instruction) {
                int jumpIndex = cp[pc + 1];

                nextPrimInsAddr = nextPrimitiveInstructionAddress(cp, wbs,
                        jumpIndex);
            } else {
                boolean condition = conditionalJumpCondition(wbs, instruction);
