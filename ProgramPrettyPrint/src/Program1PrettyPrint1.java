...
public final class Program1PrettyPrint1 extends Program1 {
    ...
    public void prettyPrint(SimpleWriter out) {
        ...
        for (Map.Pair<String, Statement> temp : tempContext) {
            String contextName = temp.key();

            printSpaces(out, INDENT_SIZE);
            out.println("INSTRUCTION " + contextName + " IS");

            Statement contextBody = temp.value();
            contextBody.prettyPrint(out, INDENT_SIZE + INDENT_SIZE);

            printSpaces(out, INDENT_SIZE);
            out.println("END " + contextName);
            out.println();
        }
